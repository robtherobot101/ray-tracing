...
{
	glm::vec3 n = normal(glm::vec3());
	//float d = glm::dot(n, (a - pt));
	return
	fabs(glm::dot(n, (a - pt))) < 0.0001 &&
	glm::dot(glm::cross(b - a, pt - a), n) > 0 &&
	glm::dot(glm::cross(c - b, pt - b), n) > 0 &&
	glm::dot(glm::cross(d - c, pt - c), n) > 0 &&
	glm::dot(glm::cross(a - d, pt - d), n) > 0;
}
